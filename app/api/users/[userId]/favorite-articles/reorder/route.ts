// File: /pages/api/users/[userId]/favorite-articles/reorder.ts
...
export async function PUT(
  ...
  try {
    ...
    // Update the position for each favorite article
    const updatePromises = list.map((item) =>
      db.favoriteArticle.update({
        where: { id: item.id },
    ...
    return new NextResponse("Favorite articles reordered successfully", { status: 200 });
  } catch (error) {
