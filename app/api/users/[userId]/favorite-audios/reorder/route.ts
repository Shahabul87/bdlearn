// File: /pages/api/users/[userId]/favorite-audios/reorder.ts
...
export async function PUT(
  ...
  try {
    ...
    // Update the position for each favorite audio
    const updatePromises = list.map((item) =>
      db.favoriteAudio.update({
        where: { id: item.id },
    ...
    return new NextResponse("Favorite audios reordered successfully", { status: 200 });
  } catch (error) {
