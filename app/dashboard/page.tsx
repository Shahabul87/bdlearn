      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="rounded-2xl p-6 bg-gradient-to-r from-blue-600 to-indigo-700 shadow-xl">
          <h1 className="text-3xl font-bold tracking-tight text-white mb-1.5">
            {hasRole(user.role, ROLES.TEACHER) && "শিক্ষক ড্যাশবোর্ড"}
            {hasRole(user.role, ROLES.PARENT) && "অভিভাবক ড্যাশবোর্ড"}
          </h1>
          <p className="text-lg text-white/90">
            {hasRole(user.role, ROLES.TEACHER) && "আপনার শিক্ষাদান কার্যক্রমের সকল তথ্য।"}
            {hasRole(user.role, ROLES.PARENT) && "আপনার সন্তানের শিক্ষা অগ্রগতি সম্পর্কে সকল তথ্য।"}
          </p>
          <p className="text-sm text-white/70 mt-1">
            {hasRole(user.role, ROLES.TEACHER) && "Access all information about your teaching activities."}
            {hasRole(user.role, ROLES.PARENT) && "See all information about your child's educational progress."}
          </p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {/* Teacher Dashboard */}
          {hasRole(user.role, ROLES.STUDENT) && (redirect("/dashboard/student"))}
         
          {hasRole(user.role, ROLES.TEACHER) && (
            <>
              <DashboardCard 
                title="আমার কোর্স" 
                titleEn="My Courses"
          ...
          {/* Parent Dashboard */}
          {hasRole(user.role, ROLES.PARENT) && (
            <>
              <DashboardCard 
          ...
          <div className="rounded-xl border border-indigo-100 dark:border-indigo-900/30 bg-gradient-to-br from-indigo-50 to-white dark:from-indigo-900/20 dark:to-gray-800/50 shadow-md p-6 transition-all duration-300 hover:shadow-lg">
            <div className="flex flex-col space-y-1.5 mb-4">
              <h3 className="text-xl font-bold text-indigo-900 dark:text-indigo-300 mb-1">
                {hasRole(user.role, ROLES.TEACHER) && "আপনার সময়সূচী"}
                {hasRole(user.role, ROLES.PARENT) && "সন্তানের শিক্ষা অগ্রগতি"}
              </h3>
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {hasRole(user.role, ROLES.TEACHER) && "Your Schedule"}
                {hasRole(user.role, ROLES.PARENT) && "Child's Education Progress"}
              </p>
              <p className="text-sm text-indigo-700 dark:text-indigo-400 mt-1">
                {hasRole(user.role, ROLES.TEACHER) && "আগামী ৭ দিনের লাইভ ক্লাস ও ইভেন্ট"}
                {hasRole(user.role, ROLES.PARENT) && "আপনার সন্তানদের সর্বশেষ কার্যকলাপ এবং ফলাফল"}
              </p>
            </div>
          ...
          <div className="rounded-xl border border-purple-100 dark:border-purple-900/30 bg-gradient-to-br from-purple-50 to-white dark:from-purple-900/20 dark:to-gray-800/50 shadow-md p-6 transition-all duration-300 hover:shadow-lg">
            <div className="flex flex-col space-y-1.5 mb-4">
              <h3 className="text-xl font-bold text-purple-900 dark:text-purple-300 mb-1">
                {hasRole(user.role, ROLES.TEACHER) && "অনিষ্পন্ন মূল্যায়ন"}
                {hasRole(user.role, ROLES.PARENT) && "সন্তানদের পরীক্ষার ফলাফল"}
              </h3>
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {hasRole(user.role, ROLES.TEACHER) && "Pending Evaluations"}
                {hasRole(user.role, ROLES.PARENT) && "Children's Exam Results"}
              </p>
              <p className="text-sm text-purple-700 dark:text-purple-400 mt-1">
                {hasRole(user.role, ROLES.TEACHER) && "শিক্ষার্থীদের জমা দেওয়া অ্যাসাইনমেন্ট মূল্যায়ন করুন"}
                {hasRole(user.role, ROLES.PARENT) && "সর্বশেষ পরীক্ষার ফলাফল দেখুন"}
              </p>
            </div>
